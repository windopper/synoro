...
export class NavigationSystemManager {
  ...
  updateNavigationSystem(state: ShipSystemsState, deltaTime: number): void {
    ...
    if (navigationSpeed <= 0) {
      console.warn('⚠️ 항법 시스템이 비활성화되어 있습니다. 기본 속도로 항법합니다.');
  ...
  private isNavigationActive(state: ShipSystemsState): boolean {
    return !!(state.navigation?.targetStarId && state.navigation?.targetPosition);
  }
  
  /**
   * 정상 항법 처리
   */
  private processNormalNavigation(state: ShipSystemsState, deltaTime: number, navigationSpeed: number): void {
    ...
    // 목표까지의 총 거리 계산
    const totalDistance = this.calculateDistance(currentPosition, targetPosition);
    ...
    const newProgress = Math.min(100, state.navigation.travelProgress + progressIncrement);
    ...
    const estimatedTimeToCompletion = remainingDistance / (effectiveSpeed / 3600); // 초 단위
   ...
   */
  private processWarpNavigation(state: ShipSystemsState, deltaTime: number): void {
   ...
   * 느린 항법 처리 (시스템 비활성화 시)
   */
  private processSlowNavigation(state: ShipSystemsState, deltaTime: number): void {
    if (!state.navigation?.targetPosition) return;
   ...
   * 효과적인 항법 속도 계산
   */
  private calculateEffectiveSpeed(state: ShipSystemsState, baseNavigationSpeed: number): number {
  ...
  private calculateNavigationModuleDamage(state: ShipSystemsState): number {
    const navigationModules = Object.values(state.installedModules)
      .filter(module => module.id.startsWith('CB_') && module.isActive);
   ...
   * 항법 중 에너지 소모 처리
   */
  private consumeNavigationEnergy(state: ShipSystemsState, deltaTime: number, effectiveSpeed: number): void {
    ...
    const energyConsumption = baseEnergyConsumption * speedMultiplier * (deltaTime / 3600);
    
    // 현재 저장된 에너지에서 차감
    const newStoredEnergy = Math.max(0, state.energy.currentStored - energyConsumption);
   ...
   * 항법 도착 시 보상 처리
   */
  private handleNavigationArrival(state: ShipSystemsState, targetStarId: string): void {
    // 도착 보상 계산
   ...
   * 도착 보상 계산
   */
  private calculateArrivalRewards(state: ShipSystemsState, targetStarId: string): {
    resources: Record<string, number>;
    researchPoints: number;
  } {
    const distance = state.navigation?.targetPosition ? 
      this.calculateDistance(state.position, state.navigation.targetPosition) : 1;
    ...
    const baseRewards = {
      resources: {
        ...
        '탐사 데이터': Math.floor(Math.random() * 3) + 1 + Math.floor(distanceBonus / 2),
    ...
    if (targetStarId.includes('binary')) {
      baseRewards.resources['이진성 데이터'] = Math.floor(Math.random() * 2) + 1;
      baseRewards.researchPoints += 15;
    } else if (targetStarId.includes('giant')) {
      baseRewards.resources['거대별 관측 데이터'] = Math.floor(Math.random() * 2) + 1;
   ...
   * 항법 중 랜덤 이벤트 체크
   */
  private checkNavigationEvents(state: ShipSystemsState, deltaTime: number): void {
  ...
  private getAvailableEvents(state: ShipSystemsState): NavigationEvent[] {
    ...
    const hasActiveCommunication = Object.values(state.installedModules).some(module => 
      module.id.startsWith('CA_') && module.isActive
    );
   ...
   * 항법 이벤트 발생
   */
  private triggerNavigationEvent(state: ShipSystemsState, event: NavigationEvent): void {
    // 이벤트 히스토리에 추가
  ...
  private handleNavigationMalfunctionEvent(state: ShipSystemsState): void {
    ...
    // 항법 관련 모듈에 경미한 손상
    const navigationModules = Object.entries(state.installedModules)
      .filter(([_, module]) => module.id.startsWith('CB_') && module.isActive);
    
    if (navigationModules.length > 0) {
      const [moduleId, _] = navigationModules[Math.floor(Math.random() * navigationModules.length)];
  ...
  getNavigationStatus(state: ShipSystemsState): NavigationStatus {
    ...
    const performance = calculateShipPerformance(state);
    const effectiveSpeed = this.calculateEffectiveSpeed(state, performance.navigation.systemSpeed);
    const totalDistance = state.navigation!.targetPosition ? 
      this.calculateDistance(state.position, state.navigation!.targetPosition) : 0;
    
    // 남은 거리와 예상 시간 계산
    const remainingDistance = totalDistance * (1 - state.navigation!.travelProgress / 100);
    const estimatedTimeRemaining = effectiveSpeed > 0 ? 
      remainingDistance / (effectiveSpeed / 3600) : Infinity; // seconds
    ...
    return {
      ...
      energyConsumptionRate: this.calculateEnergyConsumptionRate(effectiveSpeed),
 ...
 * 항법 시스템 관리자 초기화
 */
export function initializeNavigationSystem(dispatch: AppDispatch): NavigationSystemManager {
...
export function analyzeNavigationEfficiency(state: ShipSystemsState): {
  ...
  if (energyRatio < 0.3) {
    ...
    recommendations.push('에너지 저장량을 늘리거나 에너지 생산량을 증가시키세요');
  }
  ...
  const damagedModules = navigationModules.filter(module => module.currentDurability < 80);
  ...
  if (navigationModules.length === 0) {
    ...
    recommendations.push('제어 브리지 모듈을 설치하여 항법 성능을 향상시키세요');
  }
  ...
  const avgModuleDurability = navigationModules.length > 0 ? 
    navigationModules.reduce((sum, module) => sum + module.currentDurability, 0) / navigationModules.length : 0;
