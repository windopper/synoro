 ...
 * 동력 시스템 성능 계산
 */
export function calculatePowerPerformance(
  state: ShipSystemsState
): ShipPerformanceMetrics["power"] {
  ...
  Object.values(state.installedModules).forEach((module) => {
    ...
    if (performance.distributionEfficiency) {
      distributionEfficiency = Math.max(
    ...
    const energyConsumption =
      moduleInfo.energyConsumption.base +
      (moduleInfo.energyConsumption.max - moduleInfo.energyConsumption.base) *
  ...
  Object.values(state.installedModules).forEach((module) => {
    if (
      !module.isActive ||
      getModuleById(module.id)?.category === ModuleCategory.POWER
    )
      return;
  ...
  const capacityUsage =
    totalStorage > 0 ? (state.energy.currentStored / totalStorage) * 100 : 0;
 ...
 * 항행 시스템 성능 계산
 */
export function calculateNavigationPerformance(
  state: ShipSystemsState
): ShipPerformanceMetrics["navigation"] {
  let systemSpeed = 0;
  ...
  Object.values(state.installedModules).forEach((module) => {
    ...
    const moduleInfo = getModuleById(module.id);
    if (!moduleInfo || moduleInfo.category !== ModuleCategory.NAVIGATION)
      return;
    ...
    if (
      performance.warpRange &&
      performance.warpRange * damageMultiplier * allocationMultiplier >
        maxWarpRange
    ) {
      maxWarpRange =
        performance.warpRange * damageMultiplier * allocationMultiplier;
    ...
    if (performance.calculationSpeed) {
      const calculationEfficiency =
        performance.calculationSpeed * damageMultiplier * allocationMultiplier;
      navigationEfficiency = Math.max(
 ...
 * 탐사 시스템 성능 계산
 */
export function calculateExplorationPerformance(
  state: ShipSystemsState
): ShipPerformanceMetrics["exploration"] {
  let maxScanRange = 0;
  ...
  Object.values(state.installedModules).forEach((module) => {
    ...
    const moduleInfo = getModuleById(module.id);
    if (!moduleInfo || moduleInfo.category !== ModuleCategory.EXPLORATION)
      return;
    ...
    if (performance.scanResolution) {
      scanResolution +=
        (performance.scanResolution - 100) *
        damageMultiplier *
        allocationMultiplier;
    }
    ...
    if (performance.analysisAccuracy) {
      scanResolution +=
        (performance.analysisAccuracy - 100) *
        damageMultiplier *
        allocationMultiplier *
        0.5;
    }
    ...
    if (performance.dataProcessingRate) {
      analysisSpeed +=
        performance.dataProcessingRate *
        damageMultiplier *
        allocationMultiplier;
    }
    ...
    if (performance.knowledgeConversionRate) {
      knowledgeConversion +=
        (performance.knowledgeConversionRate - 100) *
        damageMultiplier *
        allocationMultiplier;
    }
    ...
    if (performance.droneCapacity) {
      totalDroneCapacity += Math.floor(
        performance.droneCapacity * damageMultiplier
      );
    }

    // 특수 드론 슬롯 (누적)
    if (performance.specialDroneSlots) {
      specialDroneSlots += Math.floor(
        performance.specialDroneSlots * damageMultiplier
      );
    }
  });
 ...
 * 통신 시스템 성능 계산
 */
export function calculateCommunicationPerformance(
  state: ShipSystemsState
): ShipPerformanceMetrics["communication"] {
  let maxCommRange = 0;
  ...
  Object.values(state.installedModules).forEach((module) => {
    ...
    const moduleInfo = getModuleById(module.id);
    if (!moduleInfo || moduleInfo.category !== ModuleCategory.COMMUNICATION)
      return;

    const performance =
      moduleInfo.performance as CommunicationModulePerformance;
    const damageMultiplier = calculateDamageMultiplier(module);
    const allocationMultiplier = module.energyAllocation / 100;
    ...
    if (performance.communicationRange !== undefined) {
      if (performance.communicationRange === 0) {
        ...
        maxCommRange = Math.max(
          maxCommRange,
          performance.communicationRange *
            damageMultiplier *
            allocationMultiplier
        );
      }
    ...
    if (performance.transmissionSpeed) {
      transmissionSpeed +=
        performance.transmissionSpeed * damageMultiplier * allocationMultiplier;
    }

    // 근거리 신호 범위 (가장 긴 범위 적용)
    if (performance.signalRange) {
      localSignalRange = Math.max(
        localSignalRange,
 ...
 * 방어 시스템 성능 계산
 */
export function calculateDefensePerformance(
  state: ShipSystemsState
): ShipPerformanceMetrics["defense"] {
  ...
  Object.values(state.installedModules).forEach((module) => {
    ...
    if (performance.shieldCapacity) {
      totalShieldCapacity +=
        performance.shieldCapacity * damageMultiplier * allocationMultiplier;
    }

    if (performance.shieldRechargeRate) {
      shieldRechargeRate +=
        performance.shieldRechargeRate *
        damageMultiplier *
        allocationMultiplier;
    }
    ...
    if (performance.repairRate && performance.selfRepair) {
      autoRepairRate +=
        performance.repairRate * damageMultiplier * allocationMultiplier;
    }
  });
 ...
 * 자원 관리 시스템 성능 계산
 */
export function calculateResourcePerformance(
  state: ShipSystemsState
): ShipPerformanceMetrics["resource"] {
  ...
  Object.values(state.installedModules).forEach((module) => {
    ...
    if (performance.extractionRate) {
      maxExtractionRate +=
        performance.extractionRate * damageMultiplier * allocationMultiplier;
    }

    // 채취 효율 (보너스 누적)
    if (performance.extractionEfficiency) {
      extractionEfficiency +=
        (performance.extractionEfficiency - 100) *
        damageMultiplier *
        allocationMultiplier;
    }
    ...
    if (performance.specialSlots) {
      specialCargoSlots += Math.floor(
        performance.specialSlots * damageMultiplier
      );
    }

    // 정제 효율 (가장 높은 효율 적용)
    if (performance.refiningEfficiency) {
      refiningEfficiency = Math.max(
        refiningEfficiency,
 ...
 */
export function autoDistributeEnergy(state: ShipSystemsState): {
...
export function analyzeModuleEfficiency(state: ShipSystemsState): {
  ...
  Object.entries(state.installedModules).forEach(([moduleId, module]) => {
    ...
    const currentEfficiency =
      damageMultiplier * (module.energyAllocation / 100) * 100;
    ...
    const upgradeRecommendation =
      (tierFactor * 0.6 + efficiencyFactor * 0.4) * 100;
    ...
    switch (moduleInfo.category) {
      ...
      case ModuleCategory.NAVIGATION:
        if (overallPerformance.navigation.systemSpeed < 100)
          bottleneckScore = 60;
        break;
      case ModuleCategory.EXPLORATION:
        if (overallPerformance.exploration.maxScanRange < 2)
          bottleneckScore = 70;
        break;
      // 다른 카테고리들도 추가...
    }
