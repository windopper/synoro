...
function isEssentialModule(moduleId: string): boolean {
  return moduleId.startsWith('FRC_') || // 핵융합로
         moduleId.startsWith('PDU_') || // 전력 분배
         moduleId.startsWith('CB_') ||  // 제어 브리지
         moduleId.startsWith('HIF_');   // 함체 프레임
}
...
function _recalculateEnergySystems(state: ShipSystemsState): void {
  ...
  Object.values(state.installedModules).forEach(module => {
    ...
    const consumption = baseConsumption + (maxConsumption - baseConsumption) * allocationMultiplier;
  ...
  state.energy.currentStored = Math.min(state.energy.currentStored, totalStorage);
}

/**
 * 미리 정의된 성능 프로파일 적용
 */
function applyPredefinedProfile(state: ShipSystemsState, profile: string): void {
  const profileSettings = {
    exploration: {
      // 탐사 모드: 스캐너와 통신 시스템 최대화
      SE_: 100, LRS_: 100, LRCA_: 100, // 탐사 시스템
      FRC_: 80, PDU_: 90, // 기본 전력
      DEF_: 30, SG_: 20   // 방어 시스템 최소화
    },
    combat: {
      // 전투 모드: 방어와 기동성 최대화
      DEF_: 100, SG_: 100, ARM_: 100, // 방어 시스템
      FRC_: 100, PDU_: 100, // 최대 전력
      SE_: 20, LRS_: 30    // 탐사 시스템 최소화
    },
    efficiency: {
      // 효율 모드: 모든 시스템 중간 수준
      default: 60
  ...
  Object.entries(state.installedModules).forEach(([moduleId, module]) => {
    ...
    // 모듈 타입별 설정 찾기
    const moduleType = Object.keys(settings).find(type => 
      type !== 'default' && moduleId.startsWith(type)
    );
    
    if (moduleType) {
      module.energyAllocation = settings[moduleType as keyof typeof settings] as number;
...
export const installModule = createAsyncThunk(
  ...
  async (params: { moduleId: string; slotPosition?: string }, { getState }) => {
    ...
    const hasResources = Object.entries(moduleInfo.requiredResources).every(
      ([resource, amount]) => (state.shipSystems.resources.inventory[resource] || 0) >= amount
    );
...
export const uninstallModule = createAsyncThunk(
  ...
  async (params: { moduleId: string; recoverResources?: boolean }, { getState }) => {
    const state = getState() as { shipSystems: ShipSystemsState };
    ...
    // 자원 회수량 계산 (내구도에 따라)
    const recoveryRate = params.recoverResources ? (module.currentDurability / moduleInfo.durability) * 0.7 : 0;
    const recoveredResources: ResourceCost = {};
    
    if (recoveryRate > 0) {
      Object.entries(moduleInfo.requiredResources).forEach(([resource, amount]) => {
...
export const runSystemDiagnostics = createAsyncThunk(
  ...
  async (_, { getState }) => {
    ...
    Object.entries(state.shipSystems.installedModules).forEach(([moduleId, module]) => {
      totalModules++;
      const moduleInfo = getModuleById(moduleId);
      if (!moduleInfo) return;
      
      const healthPercentage = (module.currentDurability / moduleInfo.durability) * 100;
      
      if (healthPercentage < 30) {
        diagnostics.criticalIssues.push(`${moduleInfo.nameKo}: 심각한 손상 (${healthPercentage.toFixed(1)}%)`);
    ...
    if (state.shipSystems.energy.emergencyMode) {
      diagnostics.recommendations.push('비상 모드가 활성화되어 있습니다. 에너지 공급을 확인하세요');
    }
...
export const startStellarExtraction = createAsyncThunk(
  ...
  async (params: { 
    ...
    const history = state.shipSystems.stellarExtraction.extractionHistory[params.star.id];
    ...
    if (history && resources.maxExtractionsPerHour) {
      ...
      const extractionsThisHour = currentHour === historyHour ? history.extractionsThisHour : 0;
      
      if (extractionsThisHour >= resources.maxExtractionsPerHour) {
        throw new Error(`이 항성에서는 시간당 최대 ${resources.maxExtractionsPerHour}회만 채취할 수 있습니다`);
      }
    }
    
    // 채취 장비 확인 (자원 채취 모듈이 있는지)
    const hasExtractionModule = Object.values(state.shipSystems.installedModules).some(module => {
      const moduleInfo = getModuleById(module.id);
      return moduleInfo?.category === ModuleCategory.RESOURCE && 
             moduleInfo.id.startsWith('RE_') && 
             module.isActive;
    });
    
    if (!hasExtractionModule) {
    ...
    // 예상 수확량 계산
    const targetResources = params.extractionType === 'primary' 
...
export const completeStellarExtraction = createAsyncThunk(
  ...
  async (params: { starId: string }, { getState }) => {
    const state = getState() as { shipSystems: ShipSystemsState };
    const extraction = state.shipSystems.stellarExtraction.activeExtractions[params.starId];
    ...
    if (!isSuccess) {
      return {
        ...
        recoveredResources: { [extraction.resourceType]: Math.floor(extraction.expectedYield * 0.3) }
      };
    }
    ...
    return {
      ...
      obtainedResources: { [extraction.resourceType]: extraction.expectedYield }
    };
  }
);
...
export const shipSystemsSlice = createSlice({
  ...
  reducers: {
    // === 기본 함선 정보 관리 ===
    updateShipPosition: (state, action: PayloadAction<{ x: number; y: number; z: number }>) => {
      state.position = action.payload;
    },
    
    moveToStar: (state, action: PayloadAction<{ starId: string; position: { x: number; y: number; z: number } }>) => {
    ...
    updateModuleEnergyAllocation: (state, action: PayloadAction<{ moduleId: string; allocation: number }>) => {
      const module = state.installedModules[action.payload.moduleId];
      if (module) {
        module.energyAllocation = Math.max(0, Math.min(100, action.payload.allocation));
        _recalculateEnergySystems(state);
      }
    },
    
    damageModule: (state, action: PayloadAction<{ moduleId: string; damage: number }>) => {
      const module = state.installedModules[action.payload.moduleId];
      if (module) {
        module.currentDurability = Math.max(0, module.currentDurability - action.payload.damage);
    ...
    repairModule: (state, action: PayloadAction<{ moduleId: string; repairAmount: number }>) => {
      const module = state.installedModules[action.payload.moduleId];
      const moduleInfo = getModuleById(action.payload.moduleId);
      
      if (module && moduleInfo) {
        module.currentDurability = Math.min(moduleInfo.durability, module.currentDurability + action.payload.repairAmount);
    ...
    // === 에너지 관리 ===
    setEnergyPriority: (state, action: PayloadAction<{ moduleId: string; priority: number }>) => {
      state.energy.prioritySettings[action.payload.moduleId] = Math.max(1, Math.min(10, action.payload.priority));
    },
    ...
    updateEnergyStorage: (state, action: PayloadAction<number>) => {
      state.energy.currentStored = Math.max(0, Math.min(state.energy.totalStorage, action.payload));
    },
    
    // === 자원 관리 ===
    addResources: (state, action: PayloadAction<ResourceCost>) => {
      Object.entries(action.payload).forEach(([resource, amount]) => {
        state.resources.inventory[resource] = (state.resources.inventory[resource] || 0) + amount;
      });
      
      // 현재 용량 재계산
      state.resources.currentCapacity = Object.values(state.resources.inventory).reduce((sum, amount) => sum + amount, 0);
    },
    
    consumeResources: (state, action: PayloadAction<ResourceCost>) => {
      Object.entries(action.payload).forEach(([resource, amount]) => {
        if (state.resources.inventory[resource]) {
          state.resources.inventory[resource] = Math.max(0, state.resources.inventory[resource] - amount);
        }
      });
    ...
    // === 탐사 관리 ===
    startScan: (state, action: PayloadAction<{ scanId: string; targetId: string; duration: number }>) => {
    ...
    updateScanProgress: (state, action: PayloadAction<{ scanId: string; progress: number }>) => {
    ...
    // === 통신 관리 ===
    updateCommunicationStatus: (state, action: PayloadAction<{ connection: boolean; signalStrength: number }>) => {
    ...
    addTransmissionToQueue: (state, action: PayloadAction<{ transmissionId: string; dataSize: number; priority: number }>) => {
      state.communicationStatus.transmissionQueue[action.payload.transmissionId] = {
    ...
    updateTransmissionProgress: (state, action: PayloadAction<{ transmissionId: string; progress: number }>) => {
      const transmission = state.communicationStatus.transmissionQueue[action.payload.transmissionId];
      if (transmission) {
        transmission.progress = Math.max(0, Math.min(100, action.payload.progress));
        
        if (transmission.progress >= 100) {
          delete state.communicationStatus.transmissionQueue[action.payload.transmissionId];
        }
      }
    },
    ...
    // === 자동화 설정 ===
    updateAutomationSettings: (state, action: PayloadAction<Partial<typeof initialState.automationSettings>>) => {
      state.automationSettings = { ...state.automationSettings, ...action.payload };
    },
    ...
    startResearch: (state, action: PayloadAction<{ techId: string; totalPoints: number }>) => {
      ...
      if (tech) {
        ...
        // 연구 포인트 소모
        state.researchPoints = Math.max(0, state.researchPoints - tech.cost.researchPoints);
        
        // 필요한 자원 소모
        if (tech.cost.requiredResources) {
          Object.entries(tech.cost.requiredResources).forEach(([resource, amount]) => {
            if (state.resources.inventory[resource]) {
              state.resources.inventory[resource] = Math.max(0, state.resources.inventory[resource] - amount);
            }
          });
    ...
    updateResearchProgress: (state, action: PayloadAction<number>) => {
      if (state.currentResearch) {
        state.currentResearch.currentPoints = Math.max(0, Math.min(state.currentResearch.totalPoints, action.payload));
    ...
    // 모듈 그룹 활성화/비활성화
    toggleModuleGroup: (state, action: PayloadAction<{ category: ModuleCategory; active: boolean }>) => {
    ...
    // 긴급 정지
    emergencyShutdown: (state, action: PayloadAction<{ excludeEssential?: boolean }>) => {
    ...
    // 성능 프로파일 적용
    applyPerformanceProfile: (state, action: PayloadAction<{
      profile: 'exploration' | 'combat' | 'efficiency' | 'stealth' | 'custom';
      customSettings?: { [moduleId: string]: { allocation: number; active: boolean } }
    }>) => {
    ...
    // === 항성 자원 채취 관리 ===
    updateExtractionProgress: (state, action: PayloadAction<{ starId: string; progress: number }>) => {
      const extraction = state.stellarExtraction.activeExtractions[action.payload.starId];
      if (extraction) {
        extraction.progress = Math.max(0, Math.min(100, action.payload.progress));
      }
    },
    ...
    // 채취 이력 업데이트
    updateExtractionHistory: (state, action: PayloadAction<{ starId: string; extractionCount?: number }>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(startModuleUpgrade.fulfilled, (state, action) => {
        ...
        // 자원 소모
        Object.entries(action.payload.requiredResources).forEach(([resource, amount]) => {
          if (state.resources.inventory[resource]) {
            state.resources.inventory[resource] -= amount;
          }
        });
      ...
      .addCase(startStellarExtraction.fulfilled, (state, action) => {
        const { starId, extractionType, resourceType, duration, expectedYield } = action.payload;
      ...
      .addCase(completeStellarExtraction.fulfilled, (state, action) => {
        const { success, starId, obtainedResources, recoveredResources } = action.payload;
