...
export const POWER_MODULES: CompleteModuleInfo[] = [
  ...
  },
  {
    ...
    requiredResources: { '제련된 트리타늄': 30, '고순도 구리': 20, 고급부품: 15 },
    upgradeCost: 200,
    durability: 120,
  ...
  },
  {
    ...
    requiredResources: { '퀀텀 크리스탈': 10, '강화 합금': 40, '정밀 제어기': 5 },
    upgradeCost: 500,
    durability: 150,
    upgradeEffects: '발전량 +100 PU, 연료 효율 +20%, 안정성 증가로 과부하 한계 +10%',
    performance: {
      powerGeneration: 250,
      fuelEfficiency: 125
    } as PowerModulePerformance
  ...
  },
  {
    ...
    tier: 1,
    category: ModuleCategory.POWER,
    description: '지능형 에너지 분배 시스템. 과부하 시 자동으로 우선순위를 관리합니다.',
    energyConsumption: { base: 8, max: 8 },
    requiredResearch: 'TECH_POWER_MGMT_01',
    requiredResources: { '초전도 배선재': 15, '제어 칩': 10, '데이터 케이블': 5 },
    upgradeCost: 150,
    durability: 100,
...
export const NAVIGATION_MODULES: CompleteModuleInfo[] = [
  ...
  },
  {
    ...
    requiredResources: { '텅스텐 합금': 25, '자기장 코일': 10, '플라즈마 인젝터': 5 },
    upgradeCost: 220,
    durability: 110,
  ...
  // 워프 드라이브 시리즈
  {
    ...
    requiredResources: { '초전도 코일': 10, '중수소 탱크': 5, '워프 안정기(기초)': 2 },
    upgradeCost: 300,
    durability: 60,
    performance: {
      warpRange: 200,
  ...
  // 항법 컴퓨터 시리즈
  {
    ...
    energyConsumption: { base: 5, max: 5 },
    requiredResources: { '실리콘 웨이퍼': 15, '데이터 케이블': 10, '기본 회로': 5 },
    durability: 85,
    performance: {
...
export const EXPLORATION_MODULES: CompleteModuleInfo[] = [
  // 장거리 스캐너 시리즈
  {
    ...
    requiredResources: { '광학 센서 어레이': 10, '신호 처리기': 5, '일반 금속': 15 },
    durability: 75,
    performance: {
      scanRange: 1,
      scanResolution: 100
  ...
  // 샘플 분석기 시리즈
  {
    ...
    energyConsumption: { base: 8, max: 8 },
    requiredResources: { '분석용 칩셋': 5, '현미경 부품': 10, '실험용 유리': 5 },
    durability: 70,
    performance: {
  ...
  // 드론 베이 시리즈
  {
    ...
    requiredResources: { '드론 격납 프레임': 1, '드론 제어기(기본)': 1, '예비 부품(드론)': 5 },
    durability: 80,
    performance: {
      droneCapacity: 1,
      specialDroneSlots: 0
  ...
  // 데이터 처리 모듈 시리즈
  {
    ...
    requiredResources: { 'CPU 코어(기본)': 2, 'RAM 모듈(기본)': 5, '데이터 저장 장치(소형)': 1 },
    durability: 85,
    performance: {
...
export const COMMUNICATION_MODULES: CompleteModuleInfo[] = [
  // 장거리 통신 시리즈
  {
    ...
    requiredResources: { '통신 안테나 어레이': 1, '신호 변조기': 2, '전력 증폭기(기본)': 3 },
    durability: 90,
    performance: {
  ...
  // 단거리 통신 시리즈
  {
    ...
    requiredResources: { '소형 안테나': 2, '저전력 트랜시버 칩': 3, '기본 필터': 1 },
    durability: 95,
    performance: {
      signalRange: 0.1
    } as CommunicationModulePerformance
...
export const DEFENSE_MODULES: CompleteModuleInfo[] = [
  // 함체 장갑 시리즈
  {
    ...
    energyConsumption: { base: 0, max: 0 },
    requiredResources: { '강화 강철판': 50, '구조 프레임': 20, '용접 키트': 10 },
    durability: 1000,
    performance: {
      hullIntegrity: 1000,
      kineticResistance: 0,
  ...
  // 방어막 시리즈
  {
    ...
    energyConsumption: { base: 10, max: 25 },
    requiredResources: { '방어막 방출기': 2, '에너지 변환 코일': 5, '기본 제어기': 3 },
    durability: 100,
    performance: {
      shieldCapacity: 300,
  ...
  // 수리 시스템 시리즈
  {
    ...
    energyConsumption: { base: 0, max: 0 },
    requiredResources: { '휴대용 용접기': 1, '예비 부품 상자': 1, '수리 매뉴얼(데이터)': 1 },
    durability: 50,
    performance: {
      repairRate: 0.1,
      selfRepair: false
...
export const RESOURCE_MODULES: CompleteModuleInfo[] = [
  ...
  // 화물칸 시리즈
  {
    ...
    energyConsumption: { base: 0, max: 0 },
    requiredResources: { '강화 컨테이너 유닛': 10, '내부 프레임': 5, '잠금 장치': 2 },
    durability: 120,
    performance: {
      storageCapacity: 500,
      specialSlots: 0
...
// 카테고리별 모듈 필터링 함수
export const getModulesByCategory = (category: ModuleCategory): CompleteModuleInfo[] => {
...
export const getUpgradePath = (moduleId: string): CompleteModuleInfo[] => {
  ...
  return ALL_MODULES.filter(m => m.id.startsWith(baseId)).sort((a, b) => a.tier - b.tier);
};

// 다음 티어 모듈 찾기 함수
export const getNextTierModule = (moduleId: string): CompleteModuleInfo | undefined => {
  const module = getModuleById(moduleId);
