...
const Star: React.FC<StarProps> = React.memo(({ star, onHover, onClick, opacity = 1 }) => {
  ...
  useFrame((state) => {
    ...
    if (material.emissiveIntensity !== undefined && star.apparentMagnitude < 1) {
      material.emissiveIntensity = 0.1 + Math.sin(state.clock.elapsedTime * 2) * 0.1 // Reduced effect
    }
  })
  ...
  const starSize = useMemo(() => {
    ...
    const baseSizeFromMagnitude = Math.max(0.35, 1.5 - star.apparentMagnitude * 0.2)
  ...
  return (
    <group position={[star.position.x, star.position.y, star.position.z]}>
      ...
      >
        <sphereGeometry args={[starSize, 8, 8]} /> {/* Reduced from 16,16 to 8,8 */}        <meshBasicMaterial 
          color={star.color}
          transparent
          opacity={0.9 * opacity}
        />
      ...
      {star.apparentMagnitude < 3 && (
        <mesh scale={hovered ? 2 : 1.5}>
          <sphereGeometry args={[starSize * 1.3, 6, 6]} /> {/* Reduced geometry */}          <meshBasicMaterial 
            color={star.color}
            transparent
      ...
      {star.apparentMagnitude < -0.5 && (
        <mesh rotation={[0, 0, Math.PI / 4]} scale={hovered ? 1.5 : 1}>
          <planeGeometry args={[starSize * 2, starSize * 0.1]} /> {/* Smaller flare */}          <meshBasicMaterial 
            color={star.color}
            transparent
            opacity={0.4 * opacity}
            depthWrite={false}
