...
interface PlanetProps {
  ...
  onHover?: (planet: PlanetData | null, position: { x: number; y: number }) => void;
...
export const Planet: React.FC<PlanetProps> = ({
  ...
  const [planetPosition, setPlanetPosition] = useState<[number, number, number]>([
    starPosition[0] + planet.orbitRadius,
    starPosition[1],
    starPosition[2]
  ])
  ...
  return (
    <group>      {/* 궤도 선 - 더 정교하고 우아한 디자인 */}
      ...
      {/* 궤도 포인트 표시 (더 미세한 점들) */}
      <mesh position={starPosition} rotation={[Math.PI / 2, 0, 0]}>
        <ringGeometry args={[planet.orbitRadius - 0.01, planet.orbitRadius + 0.01, 256]} />
        <meshBasicMaterial 
      ...
      <mesh
        ...
        onClick={handleClick}
      >
        <sphereGeometry args={[1, 32, 32]} />        <meshStandardMaterial
          color={planet.color}
          roughness={planet.type === 'gas_giant' ? 0.1 : 0.8}
          metalness={planet.type === 'rocky' ? 0.3 : 0.1}
      ...
      {planet.type === 'gas_giant' && (
        ...
        >
          <sphereGeometry args={[1, 16, 16]} />          <meshBasicMaterial
            color={planet.color}
            transparent
            opacity={0.1 * opacity}
          />
      ...
      {planet.moons && planet.moons > 0 && (
        <MoonSystem 
          planet={planet} 
          planetPosition={{ x: planetPosition[0], y: planetPosition[1], z: planetPosition[2] }}
        />
      )}
    </group>
  )
...
const MoonSystem: React.FC<{
  ...
  useFrame((state, delta) => {
    moonRefs.current.forEach((moonRef, index) => {
      ...
      const moonAngle = state.clock.elapsedTime * moonOrbitSpeed + (index * Math.PI / 2)
  ...
  return (
    <>
      {Array.from({ length: planet.moons || 0 }).map((_, index) => (
        <group key={index}>
          {/* 달 궤도 */}
          <mesh position={[planetPosition.x, planetPosition.y, planetPosition.z]} rotation={[Math.PI / 2, 0, 0]}>
            <ringGeometry args={[
