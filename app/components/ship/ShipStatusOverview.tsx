...
export default function ShipStatusOverview() {
  const spaceship = useAppSelector((state) => state.shipSystems);  const getStatusColor = (percentage: number) => {
  ...
  const computedValues = useMemo(() => {
    ...
    const fuelPercentage = Math.min(spaceship.resources?.inventory?.fuel || 0, 100);
    ...
    {/* Ship Name and Location */}
      <div className="flex flex-col">
        ...
        <div className="p-2 rounded-sm">
            ...
            <span className="text-gray-300 font-mono text-xs uppercase tracking-wider">HULL</span>
            <span className={`font-mono text-xs ${getStatusColor(computedValues.hullIntegrity)}`}>
                {computedValues.hullIntegrity.toFixed(1)}%
            </span>
            </div>
            <div className="w-full bg-gray-900/80 rounded-none h-1">
            <div
                className={`h-1 transition-all duration-300 ${getBarColor(computedValues.hullIntegrity)}`}
        ...
        <div className="p-2 rounded-sm">
            <div className="flex items-center justify-between mb-1">
            <span className="text-gray-300 font-mono text-xs uppercase tracking-wider">SHLD</span>
            <span className={`font-mono text-xs ${getStatusColor(computedValues.shieldIntegrity)}`}>
                {computedValues.shieldIntegrity.toFixed(1)}%
            </span>
            </div>
            <div className="w-full bg-gray-900/80 rounded-none h-1">
            <div
                className={`h-1 transition-all duration-300 ${getBarColor(computedValues.shieldIntegrity)}`}
        ...
        <div className="p-2 rounded-sm">
            <div className="flex items-center justify-between mb-1">
            <span className="text-gray-300 font-mono text-xs uppercase tracking-wider">PWR</span>
      ...
      <div className="border border-gray-600/20 bg-black/50 p-2 rounded-sm">
        <div className="text-gray-300 font-mono text-xs uppercase tracking-wider mb-2">RSRC</div>
        <div className="flex flex-col gap-2">
          <div>
            <div className="flex justify-between text-xs mb-1">
              <span className="text-gray-400 font-mono">FUEL</span>
              <span className="text-cyan-400 font-mono">{computedValues.fuel}</span>
            </div>
          ...
          <div>
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="text-emerald-400 font-mono">{computedValues.oxygen}%</span>
            </div>
            <div className="w-full bg-gray-900/80 rounded-none h-0.5">
              <div
                className="h-0.5 bg-emerald-500 transition-all duration-300"
                style={{ width: `${Math.min(Number(computedValues.oxygen), 100)}%` }}
              />
            </div>
          </div>
        </div>
      ...
      <div className="border border-blue-600/20 bg-black/50 p-2 rounded-sm">
        <div className="text-gray-300 font-mono text-xs uppercase tracking-wider mb-2">COMM</div>
        <div className="space-y-2">
          {/* Home Base Connection */}
          <div className="flex justify-between items-center text-xs">
            <span className="text-gray-400 font-mono">BASE</span>
            <div className="flex items-center space-x-1">
              <div className={`w-1 h-1 rounded-full ${computedValues.homeBaseConnection ? 'bg-emerald-400' : 'bg-red-400'}`} />
              <span className={`font-mono text-xs ${computedValues.homeBaseConnection ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <div>
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className={`font-mono text-xs ${getStatusColor(computedValues.signalStrength)}`}>
                {computedValues.signalStrength.toFixed(0)}%
              </span>
            </div>
            <div className="w-full bg-gray-900/80 rounded-none h-0.5">
              <div
                className={`h-0.5 transition-all duration-300 ${getBarColor(computedValues.signalStrength)}`}
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="text-cyan-400 font-mono">
              {Array.isArray(computedValues.transmissionQueue) ? computedValues.transmissionQueue.length : 0}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="border border-green-600/20 bg-black/50 p-2 rounded-sm">
        <div className="text-gray-300 font-mono text-xs uppercase tracking-wider mb-2">SYS</div>
        <div className="space-y-1">
          ...
          <div className="grid grid-cols-3 gap-1 pt-1 border-t border-gray-700/30">
            ...
            <div className="text-center">
              ...
              <div className={`w-1 h-1 rounded-full mx-auto ${computedValues.homeBaseConnection ? 'bg-emerald-400' : 'bg-amber-400'}`} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border border-purple-600/20 bg-black/50 p-2 rounded-sm col-span-2">
        <div className="text-gray-300 font-mono text-xs uppercase tracking-wider mb-2">DIAG</div>
        <div className="space-y-1">
          {computedValues.lastDiagnostics ? (
            <>
              <div className="flex justify-between items-center text-xs">
                <span className="text-gray-400 font-mono">LAST SCAN</span>
                <span className="text-gray-500 font-mono">
                  {new Date(computedValues.lastDiagnostics.timestamp || Date.now()).toLocaleTimeString()}
                </span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-gray-400 font-mono">HEALTH</span>
                <span className={`font-mono text-xs ${getStatusColor(computedValues.lastDiagnostics.overallHealth || 0)}`}>
                  {(computedValues.lastDiagnostics.overallHealth || 0).toFixed(1)}%
                </span>
              </div>
              {computedValues.lastDiagnostics.criticalIssues && computedValues.lastDiagnostics.criticalIssues.length > 0 && (
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-400 font-mono">CRITICAL</span>
                  <span className="text-red-400 font-mono">
                    {computedValues.lastDiagnostics.criticalIssues.length}
                  </span>
                </div>
              )}
              {computedValues.lastDiagnostics.recommendations && computedValues.lastDiagnostics.recommendations.length > 0 && (
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-400 font-mono">RECOM</span>
                  <span className="text-amber-400 font-mono">
            ...
            <div className="text-xs text-gray-500 font-mono">NO DIAGNOSTIC DATA</div>
          )}
        </div>
      </div>
    </div>
